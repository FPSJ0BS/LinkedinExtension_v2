...
| Check | Expected |
|---|---|
| The card on the extensions page | **Profile Vault React 3.9.1** |
| The build ID in the popup | `2026-08-25-react-v3.9.1` |
...
- `profile-vault-react-3.9.1.zip` — `INSTALL.md` beside an `extension/` folder that is a
  byte-for-byte copy of `dist/`
- `profile-vault-react-3.9.1.zip.sha256` — in the `<digest>  <name>` shape `sha256sum -c` expects
