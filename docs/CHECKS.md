...
## 3.12.0 — prime, reload once, harvest while it works (TASK-0194)
...
| `npm test` | **678 tests, 0 fail** (682 at TASK-0193; 13 policy tests replaced by 11, and the per-applicant-bound tests retired with the bound) |
...
### The requirement, driven rather than argued

*"should not stop at any number of applicants"* was tested by driving the real persisted cycle —
read the lease, plan, spend, come back with a new document — on a page recovering **one resume per
cycle**, which is the slowest shape still making progress:

| Applicants owed | Reloads | Still owed at the end |
|---|---|---|
| 10 | 10 | 0 |
| 100 | 100 | 0 |
| 500 | 500 | 0 |
...
Every size cleared completely. No count anywhere stopped it, which is the whole point — the previous
design stopped at twelve reloads regardless of list length.

### And it still terminates

| Page | Reloads | Ended with |
|---|---|---|
| primed list, one reload clears it | 1 | nothing owed |
| recovers nothing, ever | **2** | `the last reloads recovered no resume` |
| two good cycles then stalls | 4 | `the last reloads recovered no resume`, keeping the 6 recovered |

The hopeless page costs two reloads, not twelve, and the stalling page keeps everything the good
cycles won.
...
| Input | Answer |
|---|---|
| `{phase:"walking", owed:500, unproductive:0}` | `continue` — a walk in progress never reloads |
| `{phase:"finished", owed:9, unproductive:0}` | `reload / resumes-still-owed` |
| `{phase:"finished", owed:9, unproductive:1}` | `reload` — one bad cycle does not disarm it |
| `{phase:"finished", owed:9, unproductive:2}` | `give-up / the last reloads recovered no resume` |
| `{phase:"finished", owed:0}` | `continue`, empty reason |
| `{phase:"finished", owed:4, canReload:false}` | `continue / the page may not be reloaded right now` |

### The lease, driven through a full cycle

`noteResumeReload` → `readResumeReloadState`, in sequence:

| Step | `cycles` | `unproductive` | `lastOwed` |
|---|---|---|---|
| fresh entry, read at 10 owed | 0 | 0 | -1 |
| spend at 10 owed | 1 | 0 | 10 |
| read at 9 owed | 1 | **0** | 10 |
| read at 10 owed | 1 | **1** | 10 |
| read at 11 owed | 1 | 1 | 10 |

Strictly fewer is what counts as productive; the same is not. Carried through `claimAutoRun` and
`settleAutoRun` unchanged and asserted, because both rebuild the entry with a spread and a spread
that dropped these would fail silently, on a live page only.
...
| `keyCode: 13` | **0** |
| `RESUME_SCAN_STREAK_LIMIT` / `MAX_UNPRODUCTIVE_RESUME_CYCLES` | 3 / 2 |
| `MAX_RESUME_RELOADS`, `MAX_APPLICANT_RESUME_RELOADS`, `MAX_FRUITLESS_RESUME_RELOADS` | **absent** — retired, not raised |
| `dist/manifest.json` version | 3.12.0 |

### Three things the tests caught that reading had not

- **A test that measured nothing and passed.** The new mid-walk guard sliced the source between two
  markers, and the end marker (`await paceBetweenApplicants();`) occurs *earlier* in the file than
  the branch — so the slice ran backwards and produced an empty string, against which every "must
  not contain" assertion trivially passes. Now anchored to the first occurrence *after* the branch,
  with a length assertion in front of it.
- **`Infinity` fails closed, and that is right.** An unbounded failure count reads as "more failures
  than any limit" and stops the page rather than freeing it. The expectation was written the other
  way round; the code was correct and the test was wrong. Pinned explicitly, because it is a real
  arithmetic result and the direction it fails in matters.
- **`undefined` is not junk, it is absence.** A junk *limit* must clamp closed, but `undefined`
  triggers the JavaScript default parameter — which is the entire reason the default is written on
  the parameter rather than patched in afterwards. A caller omitting the limit gets the real limit,
  and that is now asserted separately from the unreadable-bound sweep.

### Not verified, and not claimed

**Rule 20 stands.** None of this has run against a live LinkedIn page; loading `dist/` in Chrome is
the user's step. Two things are worth watching on the first live run:

- **Whether one reload is enough.** The design rests on the recruiter's observation that visiting an
  applicant starts their scan. If some scans take longer than a full walk of the list, the second
  cycle picks them up — but how many cycles a real list needs is unknown here.
- **The pager after a reload.** The lease carries no pager position, so a restarted run walks from
  page one, retiring collected rows without re-opening them. That is now paid once per cycle rather
  than once per notice, which is the main reason this design is cheaper, but it is still unmeasured
  against a live pager.
...
| `tsc --noEmit` | clean |
| `npm test` | **678 tests, 678 pass, 0 fail** (668 at TASK-0188; TASK-0189 landed 10 more) |
| `check-doc-links.mjs` | 19 Markdown files, local links resolve |
| `validate-build.mjs` | 31 build files, 4 React entry points, service-worker imports |

New tests: 24 in `tests/applicant-messaging.test.js`, plus the 48 from TASK-0181
(`message-templates-core` 28, `template-store` 20).

### Run beyond the suite

The classifier was **executed**, not reasoned about, over the whole matrix:

- `MESSAGE_COMPOSE` **allows** `Message`, and `Message` + aria-label
  `Message Gaurang Agarwal` — both only with `inContainer: true`.
- `MESSAGE_COMPOSE` **refuses** `Message` outside the panel
  (`outside-applicant-panel`), `Send`, `Send message`, `InMail`, `Message all`,
  `Rate as`, `Reject` (all `forbidden-action`), `See full profile`
  (`navigates-away`), `More...` (`not-a-message-control`).
- `Message` was run against **every other** value of `CONTROL_PURPOSE` —
  contact, contact-menu, disclosure, resume, resume-download, applicant-row,
  pagination — and is refused `forbidden-action` at each.

The composer policy was executed the same way: matched by canonical profile URL
(including through a `?trk=` query) and by name; refused as
`different-recipient`, `multiple-recipients` and `unknown-recipient`; and the
insertion gate refused `message-blocked`, `empty-message`, `composer-not-empty`
and a null recipient. `composerTextMatches` returns **false** for an empty box
against intended text — the check that stops a blank insertion looking successful.

### The built artifact, not just the source

`dist/applicants.js` after `npm run build`:

- `.click()` appears **9** times — the budget, and the ninth is the Message control.
- The only synthesised keystroke in the file is **Escape** (`keyCode: 27`), used by
  `closeOpenedOverlay`. **No Enter, no `keyCode: 13`, no `form.submit`, no
  `requestSubmit`.** The composer is typed into via `execCommand("insertText")`
  and never submitted.
- No `PV_MESSAGE_SEND` or any `MESSAGE_SEND` symbol anywhere in `dist/`.

### A defect found by a test, not by reading

An agent writing the policy tests found that

```js
classifyApplicantControl({ text: "Message", ariaLabel: "Send message",
                           purpose: MESSAGE_COMPOSE, inContainer: true })
```

returned **allowed**. `label` prefers the visible text, so the anchored allowlist
never saw the verb, and the denylist exemption was rescued because the tail
pattern listed `inmail` but not `send`. The exemption's own comment — claiming a
Send control could never match an allowlist anchored on `message` — was therefore
false for exactly the shape that mattered. `send`, `sends`, `sending`, `submit`
and `deliver` were added to the tail, which is now asked of the label **and** the
accessible name together. Asserted in both test files.

### Intermediate failures worth recording

Twelve existing tests went red against the click-policy change, and each was a
real pin rather than noise: the eight-click budget in twenty assertions, the
named-owner list (`insertApplicantMessage` had to be added), the blanket "Message
must be refused for every purpose" at `tests/applicants-core.test.js:91`, and
`Phase 9`, whose slice runs from `captureApplicantUi` to the message handler — the
composer section landed between those two markers, so the capture test began
reporting the composer's click as the capture's. Re-bounded on the composer's
first declaration, which survives `withoutComments`; a comment marker would not.

### Not verified

**Nothing here has run against a live LinkedIn page.** Rule 20 stands: fixtures
are not the live DOM, and loading `dist/` in Chrome is the user's step. This
matters more than usual for this feature — every other unverified path costs a
blank CSV cell, and this one reaches a person. See
[applicant-messaging-guide.md](applicant-messaging-guide.md).
