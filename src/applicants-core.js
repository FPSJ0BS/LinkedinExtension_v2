...
(() => {
   ...
   * fingerprint was `applicationId | text.length | first 300 chars`, and the
   * caller's whole test was *"that string is not what it was before the click"*.
   * Two of those three fields are text — and **the teardown alone satisfies
   * it**. The moment LinkedIn tears the previous applicant's panel out, the text
   * changes, the wait returns, and the scan starts on an empty or half-built
   * column. "The content stopped growing" is then trivially true, so it settles
   * at once and moves on. It looks exactly like a short fixed timer, and it is
   * why a profile is left before it has loaded.
   *
   * The question the caller is actually asking has an exact answer and it is not
   * a diff: **is the panel showing the application this row leads to, and has it
   * finished mounting?** Both halves are structural. The id comes from the
   ...
   * slug — and still never from text, compared against the identity the panel
   * had before the click. It can only ever say "somebody else is here now",
   * which is weaker than the id test and is why the id test is preferred, but it
   * is not a guess either and a run must not stall on a layout that numbers its
   * rows differently.
   */
