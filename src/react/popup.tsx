...
const EXPECTED_BUILD_ID = "2026-09-02-react-v3.14.1";
...
class PopupApp extends React.Component {
   ...
   *
   * The whole-job command on this surface. It is here as well as on the
   * Applicants page because the popup is what the recruiter has open while they
   * are *on* the hiring tab, which is the one place it is started from.
   *
   * **Collect Every Applicant was removed in 3.7.13** — it rode this same
   * message, this same walk and this same handler, differing only in the flag
   * below, and by then this pass opened each applicant, disclosed their contact
   * details and saved their resume anyway. `listOnly: true` still travels with
   * the armed options rather than being dropped, so a run armed by the previous
   * build resumes as what it is instead of falling into a branch that no longer
   * exists.
   */
   ...
   *
   * This is the command that closes the popup itself. The run happens on the
   * hiring tab, which the worker has just activated and focused, so a popup
