...
(() => {
  ...
  const state = {
    ...
    running: null,
    /**
     * True while an insertion into the composer is in flight (3.10.0).
     *
     * Declared here rather than left to spring into existence, because it is a
     * mutual exclusion and its absence is what it means: one applicant, one
     * composer, one insertion at a time. A second request arriving mid-flight is
     * refused rather than queued — there is no queue anywhere in this feature.
     */
    messaging: false,
  ...
  async function extractAllApplicants(options = {}) {
    ...
    /**
     * THE END OF THE WALK IS THE SECOND TRIGGER, and it is now the WIDER one.
     *
     * The trigger above answers each notice where it appears, and bounded per
     * applicant it necessarily leaves some behind: the applicant whose own two
     * reloads were spent, everyone passed over while the page could not be
     * reloaded, and everyone whose notice arrived after the job's ceiling. This
     * is the sweep for all of them. By now every cheap remedy has had its turn
     * and minutes have passed, so a scan that was genuinely still running has
     * had every chance to finish and what is left is the document.
     *
  ...
  // ------------------------------------------- the applicant's own composer
  //
  // The recruiter writes a message in this extension's dashboard, previews it
  // against the applicant's own collected record, and this types it into
  // LinkedIn's composer. **Nothing here sends.** The composer LinkedIn opens
  // says "Press Enter to Send" and the human presses it; there is no Send
  // control resolved, found or pressed anywhere in this file, and `send` is on
  // the denylist for every purpose, so none could be.

  /**
   * The composer's editable box.
   *
   * **`isExcludedContext` is deliberately not consulted here, and that is the
   * whole reason this resolver exists separately.** That helper excludes
   * `.msg-overlay-conversation-bubble` and `.msg-overlay-list-bubble` — which
   * is exactly what the composer mounts as — and it is right to, everywhere
   * else: a messaging overlay floating over the applicant list is not part of
   * the applicant, and reading it would attribute a stranger's conversation to
   * whoever is on screen. This is the one case CLAUDE.md rule 6 names as the
   * exception: an overlay this extension opened itself, addressed to the
   * applicant it just proved the panel is showing.
   *
   * Ordered most-specific first, so LinkedIn's own message form wins over a
   * generic editable box.
   */
  const COMPOSER_BOX_SELECTOR = [
  ...
  /** The surface around the box that carries who it is addressed to. */
  ...
  /** Where a recipient is NAMED, as the captured layout renders it. */
  const COMPOSER_RECIPIENT_SELECTOR = [
    "[class*='added-recipient']",
    "[class*='typeahead__added']",
    "[class*='msg-compose__recipient']",
    "[class*='recipient']"
  ].join(",");

  function composerBox() {
    for (const box of document.querySelectorAll(COMPOSER_BOX_SELECTOR)) {
      if (isVisible(box)) return box;
    }
    return null;
  }

  function composerSurface(box) {
    return box?.closest(COMPOSER_SURFACE_SELECTOR) || box?.parentElement || null;
  }

  /** What the box currently holds, as the human sees it. */
  function composerText(box) {
    return cleanText(box?.innerText || box?.textContent || "");
  }

  /**
   * Who the composer says it is addressed to.
   *
   * The recipient pill is asked FIRST and on its own — in the captured layout
   * that is the green `Gaurang Agarwal ✕` chip, and it names the recipient and
   * nobody else. Only when there is no pill does this fall back to the profile
   * links in the surface, which is a weaker reading: a conversation header can
   * carry the sender's own avatar link too, and two distinct members found here
   * is reported as exactly that. The core refuses on ambiguity rather than
   * picking one, because picking wrong sends one applicant's message to
   * another.
   */
  function composerRecipients(box) {
    const surface = composerSurface(box);
    if (!surface) return { names: [], profileUrls: [], source: "no-surface" };

    const pills = [...surface.querySelectorAll(COMPOSER_RECIPIENT_SELECTOR)].filter(isVisible);
    if (pills.length) {
      return {
        // The pill renders its own dismiss glyph beside the name; the core
        // normalizes, and a trailing ✕ never becomes part of a member's name.
        names: pills.map((pill) => cleanText(pill.innerText || pill.textContent).replace(/[✕✖×x]\s*$/i, "")),
        profileUrls: pills.flatMap((pill) => [...pill.querySelectorAll("a[href*='/in/']")].map((a) => a.getAttribute("href") || "")),
        source: "recipient-pill"
      };
    }
    const anchors = [...surface.querySelectorAll("a[href*='/in/']")].filter(isVisible);
    return {
      names: anchors.map((anchor) => cleanText(anchor.innerText || anchor.textContent)).filter(Boolean),
      profileUrls: anchors.map((anchor) => anchor.getAttribute("href") || ""),
      source: anchors.length ? "profile-link" : "unnamed"
    };
  }

  /** Is the panel showing the applicant this request names? */
  function panelIsShowing(applicant) {
    const panel = arrivalPanel();
    if (!panel) return { panel: null, matched: false, reason: "no-panel", identity: "" };

    const identity = panelIdentity(panel);
    const wantUrl = Core.canonicalizeProfileUrl(applicant?.profileUrl || "") || "";
    const wantId = cleanText(applicant?.applicationId || "");
    const haveUrl = panelMemberUrl(panel);
    const haveId = panelOwnApplicationId(panel);

    if (!haveUrl && !haveId) return { panel, matched: false, reason: "panel-unidentified", identity };
    // At least one identifier must POSITIVELY agree. Silence is never consent
    // here: an unidentifiable panel is refused rather than assumed to be the
    // right person, for the same reason the panel reader refuses a record it
    // cannot attribute.
    const agrees = (wantUrl && haveUrl && wantUrl === haveUrl) || (wantId && haveId && wantId === haveId);
    if (!agrees) return { panel, matched: false, reason: "panel-shows-other-applicant", identity };
    return { panel, matched: true, reason: "panel-matches", identity };
  }

  const COMPOSER_OPEN_TIMEOUT_MS = 8000;

  /**
   * Type the recruiter's reviewed text into the composer, and stop.
   *
   * `execCommand("insertText")` is the primary path for one reason that matters:
   * the box is a React-controlled `contenteditable`, and assigning `textContent`
   * updates what is painted without telling React anything. LinkedIn would then
   * send the empty string its own state still holds. `insertText` generates the
   * real `beforeinput`/`input` events React is listening for, so its state and
   * the pixels agree.
   *
   * The fallback exists only for the day that command is gone, and it is
   * reported as a fallback rather than quietly used, because the read-back below
   * can only prove what the DOM shows — not what React believes.
   */
  function insertComposerText(box, text) {
    ...
    if (ok) return "insert-text";

    box.textContent = text;
    box.dispatchEvent(new InputEvent("input", { bubbles: true, inputType: "insertText", data: text }));
    return "fallback-input-event";
  }

  /** Observation only: what a compose request WOULD find. Nothing is pressed. */
  function probeMessageComposer(applicant) {
    const showing = panelIsShowing(applicant);
    const box = composerBox();
    const control = showing.panel
      ? findControl(showing.panel, Applicants.CONTROL_PURPOSE.MESSAGE_COMPOSE)
      : null;
    return {
      ok: true,
      buildId: BUILD_ID,
      surface: "applicants",
      panel: Boolean(showing.panel),
      panelMatches: showing.matched,
      panelReason: showing.reason,
      identity: showing.identity,
      control: {
        found: Boolean(control),
        label: control?.verdict?.label || "",
        allowed: Boolean(control?.verdict?.allowed),
        reason: control?.verdict?.reason || "no-message-control"
      },
      composerOpen: Boolean(box),
      composerEmpty: box ? !composerText(box) : true
    };
  }

  /**
   * The whole flow: prove the applicant, open their composer, type, verify.
   *
   * Every refusal below leaves the page exactly as it was found. The composer is
   * never closed on any path — unlike the overflow menu, which is closed on
   * every exit because a menu left open is the next applicant's problem. A
   * composer is the opposite: it is the human's workspace, it may hold a draft
   * they wrote, and it is the thing they are about to press Enter in.
   */
  async function insertApplicantMessage({ applicant = {}, text = "" }) {
    // Every key declared up front, so an early return reports `false` rather
    // than omitting the field and leaving the dashboard to guess (3.9.1).
    const diagnostics = {
      panel: false, panelReason: "", identity: "",
      controlFound: false, controlLabel: "", controlReason: "",
      clicked: false, composerFound: false, composerReused: false,
      recipientSource: "", recipientReason: "", recipientOn: "",
      existingText: false, insertedVia: "", readBack: false
    };
    const done = (inserted, reason) => ({ inserted, reason, diagnostics });

    if (!isPageVisible()) return done(false, "page-hidden");
    if (state.messaging) return done(false, "another-insertion-in-progress");
    state.messaging = true;
    try {
      const showing = panelIsShowing(applicant);
      diagnostics.panel = Boolean(showing.panel);
      diagnostics.panelReason = showing.reason;
      diagnostics.identity = showing.identity;
      if (!showing.matched) return done(false, showing.reason);

      // An already-open composer is reused when it is addressed to this
      // applicant — that costs no click at all, and re-pressing Message would
      // stack a second composer over the recruiter's own open draft.
      let box = composerBox();
      let recipient = box
        ? Applicants.verifyComposerRecipient({ expected: applicant, observed: composerRecipients(box) })
        : { matched: false, reason: "no-composer" };
      diagnostics.composerReused = Boolean(box && recipient.matched);

      if (!diagnostics.composerReused) {
        const control = findControl(showing.panel, Applicants.CONTROL_PURPOSE.MESSAGE_COMPOSE);
        diagnostics.controlFound = Boolean(control);
        diagnostics.controlLabel = control?.verdict?.label || "";
        diagnostics.controlReason = control?.verdict?.reason || "no-message-control";
        if (!control) return done(false, "no-message-control");

        try {
          control.element.click();
          diagnostics.clicked = true;
        } catch (error) {
          return done(false, `message-click-failed:${error?.message || error}`);
        }

        // Wait for a composer that is addressed to THIS applicant, not merely
        // for a composer. The messaging overlay persists across applicants, so
        // the previous conversation is routinely what is on screen first.
        box = await waitFor(() => {
          const candidate = composerBox();
          if (!candidate) return null;
          const who = Applicants.verifyComposerRecipient({
            expected: applicant,
            observed: composerRecipients(candidate)
          });
          return who.matched ? candidate : null;
        }, { timeoutMs: COMPOSER_OPEN_TIMEOUT_MS, label: "message-composer" });

        if (!box) {
          const fallback = composerBox();
          const who = fallback
            ? Applicants.verifyComposerRecipient({ expected: applicant, observed: composerRecipients(fallback) })
            : { reason: "composer-never-opened" };
          diagnostics.composerFound = Boolean(fallback);
          diagnostics.recipientReason = who.reason || "";
          return done(false, fallback ? who.reason : "composer-never-opened");
        }
        recipient = Applicants.verifyComposerRecipient({ expected: applicant, observed: composerRecipients(box) });
      }

      diagnostics.composerFound = true;
      const observed = composerRecipients(box);
      diagnostics.recipientSource = observed.source;
      diagnostics.recipientReason = recipient.reason;
      diagnostics.recipientOn = recipient.on || "";

      const existing = composerText(box);
      diagnostics.existingText = Boolean(existing);
      ...
      if (!plan.allowed) return done(false, plan.reason);

      diagnostics.insertedVia = insertComposerText(box, plan.text);

      // THE LAST GATE. What is in the box now must be what was approved — if
      // the insertion half-landed, or landed somewhere else, the recruiter must
      // not be handed a composer that looks ready and holds the wrong words.
      // There is nothing to undo on failure: the text is reported as not
      // inserted and the human is looking at the box.
      diagnostics.readBack = Applicants.composerTextMatches(composerText(box), plan.text);
      return diagnostics.readBack
        ? done(true, "inserted")
        : done(false, "read-back-mismatch");
    } finally {
  ...
  state.handler = (message, _sender, sendResponse) => {
    ...
    // Observation only — this branch presses nothing, so it is answered
    // synchronously. It is what lets the dashboard say "the panel is showing
    // somebody else" BEFORE the recruiter writes a message for the wrong person.
    if (type === "PV_APPLICANT_MESSAGE_PROBE") {
      sendResponse(probeMessageComposer(message.applicant || {}));
      return false;
    }

    if (type === "PV_APPLICANT_MESSAGE_INSERT") {
      insertApplicantMessage({ applicant: message.applicant || {}, text: message.text || "" })
        .then((result) => sendResponse({
          // `ok` reports that the request was HANDLED; `inserted` reports what
          // happened. A refused insertion is a successful answer to a question,
          // not a failure of the extension, and the dashboard shows `reason`
          // either way rather than a generic "something went wrong".
          ok: true,
        ...
        .catch((error) => sendResponse({
          ok: false,
          inserted: false,
          reason: error?.stopped ? "stopped" : error?.hidden ? "page-hidden" : "insert-failed",
