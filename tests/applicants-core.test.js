...
test("the walk waits for conditions instead of sleeping over them, and misses nothing for it", async () => {
  const source = await readFile(resolve(root, "applicants.js"), "utf8");
  const worker = await readFile(resolve(root, "src/background.ts"), "utf8");
  const bare = withoutComments(source);

  // 1. THE COMPLETENESS FLOORS ARE UNTOUCHED, and that is the whole licence for
  //    everything else in this test. A shorter quiet window can only take a read
  //    a moment early — merge-only accumulator, every walk re-reads every pass —
  //    but what decides a walk is FINISHED is a pass count and the bottom, and
  //    none of it may move to make the run feel faster.
  assert.match(source, /const REVEAL_MIN_PASSES = 4;/, "the four-pass reveal floor stands");
  assert.match(source, /const REVEAL_QUIET_PASSES = 3;/, "so does the three-quiet-pass rule");
  assert.match(source, /const REVEAL_MAX_PASSES = 40;/, "and the reveal's own ceiling");
  assert.match(source, /const LIST_QUIET_PASSES = 3;/, "the list still confirms its bottom three times");
  assert.match(source, /const LIST_PAGE_PASSES = 24;/, "and still walks the whole page to find it");
  assert.match(source, /if \(reachedTail && quiet >= REVEAL_QUIET_PASSES && record\.passes >= REVEAL_MIN_PASSES\)/,
    "the panel walk still ends only at the bottom, quiet, and past its floor");

  // 2. A PDF VIEWER PAINTING ITSELF IS NOT THE PAGE STRUGGLING. `recordTempo`
  //    drops the whole run to SLOW on ONE unsettled sample and keeps it there,
  //    so a wait held over this extension's own overlay — which mutates by
  //    design and therefore always times out — was convicting the hiring page
  //    and taxing every later applicant for it.
  assert.match(source, /function waitForDomQuiet\(quietMs = \d+, timeoutMs = \d+, \{ feedsTempo = true \} = \{\}\)/,
    "a wait can be held over something that is not the page");
  assert.match(source, /if \(feedsTempo\) recordTempo\(sample\);/,
    "and such a wait is still SIZED by the tempo, it simply does not vote on it");
  const viewerWaits = (bare.match(/waitForDomQuiet\([^)]*\{ feedsTempo: false \}\)/g) || []).length;
  assert.equal(viewerWaits, 2, "exactly the two waits held over the resume viewer opt out");
  // Defaulting to true is what keeps this an exception rather than a loophole.
  const pageWaits = (bare.match(/await waitForDomQuiet\(\d+, \d+\);/g) || []).length;
  assert.ok(pageWaits >= 10, "every other wait still votes on the page's tempo");

  // 3. NO FIXED SLEEP IN FRONT OF A POLL-UNTIL-TRUE. Pressing Download used to
  //    sleep up to 900ms so the request "would exist" — in front of a `waitFor`
  //    over RESUME_DOCUMENT_TIMEOUT_MS that catches the entry the instant
  //    `watchResumeRequests` delivers it. Being early there costs one poll
  //    interval, never a file.
  const press = bare.slice(bare.indexOf("async function clickResumeDownload"), bare.indexOf("async function resolveResumeDocumentUrl"));
  assert.ok(!/waitForDomQuiet\(150, 900\)/.test(press), "the long blind sleep after Download is gone");
  assert.match(press, /await waitForDomQuiet\(60, 250, \{ feedsTempo: false \}\);/,
    "replaced by a beat for the click's own turn of the event loop");
  assert.match(bare, /timeoutMs: RESUME_DOCUMENT_TIMEOUT_MS/, "and the document is still POLLED for, on the caller's own ceiling");

  // 4. A DISMISSED OVERLAY IS LOOKED AT, NOT SLEPT OVER. Both dismiss steps used
  //    to `wait(250)` and only then look, twice per attempt, per applicant.
  const closer = bare.slice(bare.indexOf("async function overlayClosed"), bare.indexOf("async function closeOpenedOverlay"));
  assert.match(closer, /await wait\(Math\.min\(OVERLAY_CLOSE_POLL_MS, budgetMs - waited\)\)/, "it polls within the budget");
  assert.match(closer, /if \(gone\(\)\) return true;/, "and returns the moment the overlay is off the page");
  const dismiss = bare.slice(bare.indexOf("async function closeOpenedOverlay"), bare.indexOf("function findContactDisclosure"));
  assert.ok(!/await wait\(250\)/.test(dismiss), "no blind quarter-second remains on the dismiss path");
  // The budget itself is unchanged: an overlay that genuinely refuses still gets
  // its full 250ms before the next tactic, and the verdict is still "is it there".
  assert.match(source, /async function overlayClosed\(overlay, budgetMs = 250\)/, "the budget a stubborn overlay gets is unchanged");
  assert.match(dismiss, /for \(let attempt = 0; attempt < 3; attempt \+= 1\)/, "and so are the three attempts");

  // 5. THE LIST IS NOT SCROLLED TO FIND A ROW THAT IS ALREADY ON SCREEN — and
  //    the page-settling callers are untouched, because page membership and the
  //    pager gate (rule 9h, 3.7.12) rest entirely on them starting at the top.
  const sweep = bare.slice(bare.indexOf("async function sweepCurrentPage"), bare.indexOf("async function growApplicantList"));
  const early = sweep.indexOf("if (typeof wanted === \"function\" && wanted()) {");
  const top = sweep.indexOf("scrollPanelTo(0, chooseScrollTarget(list));");
  assert.ok(early > -1 && top > -1, "both the early answer and the top-scroll are present");
  assert.ok(early < top, "a caller asking for one row is answered BEFORE the list is scrolled anywhere");
  assert.match(sweep, /scrollPanelTo\(0, chooseScrollTarget\(list\)\);/,
    "and a page-settling call still starts at the top, every time");
  // It still presses nothing and still never looks for the pager.
  assert.ok(!/\.click\(\)/.test(sweep), "settling a page presses nothing");
  assert.ok(!/findApplicantPaginationControl/.test(sweep), "and never so much as looks for the pager");

  // 6. THE WORKER READS THE PATH BACK ON AN ESCALATING POLL, and the content
  //    script blocks on that reply, so its floor was spent once per applicant.
  assert.match(worker, /let interval = 25;/, "the first look comes quickly");
  assert.match(worker, /interval = Math\.min\(200, Math\.round\(interval \* 1\.6\)\);/, "then backs off");
  assert.match(worker, /if \(attempt < PATH_POLLS - 1\)/, "and never sleeps after the last look");
  assert.match(worker, /if \(item && item\.state === "interrupted"\)/,
    "an interrupted download is still reported as interrupted, never as saved");

  // 7. AND NOT ONE CONTROL WAS ADDED TO MAKE ANY OF THIS FASTER.
  assert.equal((bare.match(/\.click\(\)/g) || []).length, 7, "the seven-click budget is untouched");
});
