...
test("every control that acts on the applicant is refused before any allowlist", () => {
  ...
  for (const label of forbidden) {
    for (const purpose of Object.values(Applicants.CONTROL_PURPOSE)) {
      // The 3.14.0 carve-out, and the only one: `message` is forgiven at
      // MESSAGE_OPEN and `send` at MESSAGE_SEND, because the user asked for
      // templated messages and rule 5 was amended for them. Skipped here in the
      // same shape the APPLICANT_ROW exemption is skipped further down, and
      // bounded by the PERMANENT test that walks every purpose against these
      // same labels and proves the exemption reaches nothing else — including
      // "Send InMail" and "Message and reject", which stay refused AT the two
      // message purposes.
      if (purpose === Applicants.CONTROL_PURPOSE.MESSAGE_OPEN) continue;
      if (purpose === Applicants.CONTROL_PURPOSE.MESSAGE_SEND) continue;
...
test("the applicants adapter clicks only its gated controls", async () => {
  ...
  //
  // The tenth and eleventh arrived together in 3.14.0: `clickMessageOpen`
  // presses the panel's Message control and `clickMessageSend` presses the
  // composer's own Send. They are the only clicks in this extension that cannot
  // be undone by a later step, which is why they are two owners rather than one
  // shared presser — the tripwire's value is naming WHICH control a site
  // belongs to, and "a message was opened" and "a message was sent" are not the
  // same event to be wrong about. Rules 5 and 6 were amended in the same task.
  assert.equal(clicks.length, 11, `only ten gated controls and one dismiss may be clicked, found ${clicks.length}`);
  assert.equal(
...
test("the run walks the list by identity, so a position can never address the wrong person", async () => {
  ...
  assert.equal((source.match(/\.click\(\)/g) || []).length, 11, "the click budget is eleven");
});
...
test("PERMANENT: one click per applicant, wait for the right panel, scroll only that column", async () => {
  ...
  // protected from.
  assert.match(recoveryBranch, /applicantKey: result\.resumeRecovery\.applicantKey/,
    "and it is charged to the applicant whose notice triggered it");
  assert.ok(
...
test("a row that would not open is opened again, because nothing was read when it did not", async () => {
  ...
  assert.equal((source.match(/\.click\(\)/g) || []).length, 11, "and still exactly eleven in the file");
});
...
test("the numbered pager is identified as a group, and never pressed on a guess", async () => {
  ...
  assert.equal((source.match(/\.click\(\)/g) || []).length, 11, "still exactly eleven clicks");
...
test("Collect Every Applicant is gone, and it took nothing else with it", async () => {
  ...
  // removed here — only a caller. Eleven: ten gated opens plus one shared
  // dismiss.
  assert.equal((source.match(/\.click\(\)/g) || []).length, 11,
    "applicants.js must still click exactly eleven times");
});
...
test("Phase 1 tripwire: nine clicks, and each one is owned by a named function", async () => {
  ...
  const owners = [
    ...
    // The two from 3.14.0, and the only presses here that no later step can
    // undo. Everything above this line reads; these two write to a stranger.
    "clickMessageOpen",        // the panel's Message control (rule 9l)
    "clickMessageSend"         // the composer's own Send (rule 9m)
  ];

  assert.equal((source.match(/\.click\(\)/g) || []).length, owners.length,
    "the click budget is eleven: ten gated opens and one shared dismiss");

  const declarations = adapterFunctions(source);
  const found = [...source.matchAll(/\.click\(\)/g)].map((match) => ownerOf(declarations, match.index));

  assert.deepEqual([...found].sort(), [...owners].sort(),
    "every click is owned by exactly one of the eleven sanctioned functions — none may move house");
});
...
test("Phase 4: the layout verdict reaches the dispatch and the diagnostics, and nothing else", async () => {
  ...
  assert.equal((source.match(/\.click\(\)/g) || []).length, 11, "the click budget is still eleven");
});
...
test("Phase 7: whether the left list moved is measured on the page, because it cannot be measured here", async () => {
  ...
  assert.equal((source.match(/\.click\(\)/g) || []).length, 11, "and the click budget is still eleven");
});
...
test("Phase 11: every resume variant is reached without a click of its own", async () => {
  ...
  assert.deepEqual(
    ...
    ["clickApplicantPager", "clickMessageOpen", "clickMessageSend", "clickResumeDownload", "closeOpenedOverlay", "collectResume", "expandCollapsedSections", "openContactAndCollect", "openContactMenu", "resumeAddressFromCard", "selectApplicantRow"],
...
test("Phase 12: twelve phases changed no schema, no CSV and no click", async () => {
  ...
  assert.equal((source.match(/\.click\(\)/g) || []).length, 11, "eleven clicks in, eleven clicks out");
...
test("3.9.1: the eighth click is a new control, and it is the only new one", async () => {
  ...
  assert.equal((source.match(/\.click\(\)/g) || []).length, 11, "ten gated opens and one shared dismiss");
  assert.equal((source.match(/control\.element\.click\(\)/g) || []).length, 8,
    "the verdict-gated opens are the five of 3.9.1, the card's Download (3.13.0), "
    + "and the Message and Send of 3.14.0");
...
test("3.9.3: the opener can never be returned as its own menu", async () => {
  ...
  assert.equal((source.match(/\.click\(\)/g) || []).length, 11, "and the file's budget is unchanged");
});
...
test("3.9.3: a whole-job run expands collapsed sections, on a smaller budget than a single collection", async () => {
  ...
  assert.equal((source.match(/\.click\(\)/g) || []).length, 11, "the click budget is still eleven sites");
});
...
test("3.9.4: every other purpose still tests the whole string, aria-label included", () => {
  ...
  for (const purpose of Object.values(Applicants.CONTROL_PURPOSE)) {
    ...
    // And the 3.14.0 exemption, for the same reason the row is skipped: this
    // list contains "Message", "Message Gaurang" and "Send", which are now the
    // two controls rule 5 was amended to allow. Every OTHER label on the list —
    // "Send InMail" and "InMail" included — is still refused at both message
    // purposes, and the PERMANENT test below is where that is proved rather
    // than assumed.
    if (purpose === Applicants.CONTROL_PURPOSE.MESSAGE_OPEN) continue;
    if (purpose === Applicants.CONTROL_PURPOSE.MESSAGE_SEND) continue;
    for (const label of FORBIDDEN_ACTION_LABELS) {
...
test("3.11.0: the notice is answered on the applicant that shows it, not two applicants later", () => {
  const { planResumeRecovery: plan, RESUME_RECOVERY } = Applicants;

  // AMENDED FROM 3.10.0, IN THE OPEN, AND THE AMENDMENT IS THE POINT OF THE
  // RELEASE. This test was called "the reload answers evidence the DOCUMENT is
  // stale, never a single notice", and it asserted that ONE notice must not
  // reload: one is an ordinary per-file scan, three in a row is a stale page.
  // The recruiter watched it run and reported what that judgement cost:
  //
  //   "I want to reload the page as soon as the error is seen. While checking
  //    it I am seeing it NEVER RELOAD ON THE SAME PROFILE IT OCCURS - the
  //    extension is reloading after the 2nd or 3rd, and at last it was not even
  //    reloading, it kept going through 6 profiles without even reloading."
  //
  // A remedy that arrives two applicants after the page it is fixing is one
  // nobody watching can see working, and the applicant in the screenshot was
  // never the one it helped.
  //
  // NOTHING WAS GIVEN UP TO GET HERE. What the streak was really buying is
  // loop-safety — a file whose scan has genuinely not finished must not send the
  // page round for ever — and that is now bought directly by
  // MAX_APPLICANT_RESUME_RELOADS, asserted in its own test below. The streak
  // bought it by making two innocent applicants fail first; the per-applicant
  // bound buys it from the applicant who is actually causing it.

  // Still true, and still the guarantee that protects every healthy run: a page
  // whose resumes are fine is left alone.
  ...
  // ONE notice now reloads, and `page-stale` still names the thing being blamed.
  // "One notice" is not "the instant the notice paints": `waitForResumeScan`
  // polls it for five seconds first and returns early the moment it clears, so
  // what reaches this policy is a notice that already survived the cheap remedy.
  assert.deepEqual(plan({ phase: "walking", streak: 1, owed: 1 }),
    { action: RESUME_RECOVERY.RELOAD, reason: "page-stale", owed: 1, reloads: 0 },
    "the notice is answered where it appears, on that applicant");

  // The end of the walk is the other trigger and needs no streak at all. It is
  // now the WIDER of the two: reloading on sight is bounded per applicant, so it
  // necessarily leaves people behind, and this is the sweep for all of them.
  ...
  // THE JOB CEILING, WHICH NO LONGER EXISTS. Twelve reloads used to be the end
  // of recovery for the whole job; it is now just a number of reloads that have
  // happened, reported and not judged.
  assert.deepEqual(plan({ phase: "finished", owed: 4, reloads: 12 }),
    { action: RESUME_RECOVERY.RELOAD, reason: "resumes-still-owed", owed: 4, reloads: 12 },
    "a job still owed files keeps going, however many reloads it has taken");

  // This applicant's own reloads, spent. The most specific answer available, so
  // it is the one reported: "this person's file is not coming back however often
  // the page is reloaded" is a better reason than "the job is out of budget".
  assert.deepEqual(plan({ phase: "walking", streak: 1, owed: 2, applicantReloads: 2 }),
    { action: RESUME_RECOVERY.GIVE_UP, reason: "this applicant's reloads are spent", owed: 2, reloads: 0 },
    "one applicant may not reload the page for ever");

  // Reloads that recovered nothing, three deep. Replaces 3.10.0's rule, which
  // gave up after ONE fruitless reload and is what "at last it was not even
  // reloading" actually was.
  // THE FRUITLESS BREAKER, WHICH IS GONE. Three reloads that recovered nothing
  // used to end recovery for the entire job; it now says nothing at all, because
  // "three" is a count and a count cannot know how long the list is.
  assert.deepEqual(plan({ phase: "finished", owed: 4, reloads: 3, fruitless: 3 }),
    { action: RESUME_RECOVERY.RELOAD, reason: "resumes-still-owed", owed: 4, reloads: 3 },
    "an unhelpful run of reloads no longer disarms the job for everyone after it");
  ...
  // `recovered` WAS the input that decided this and is now ignored entirely: a
  // per-run counter cannot say whether reloads help ACROSS a reload, because the
  // reload destroys the run holding it. Passing it must change nothing.
  assert.deepEqual(plan({ phase: "finished", owed: 4, reloads: 1, recovered: 0 }),
    plan({ phase: "finished", owed: 4, reloads: 1 }),
    "the retired parameter cannot influence the verdict from beyond the grave");
});

test("3.11.0: PERMANENT — a quiet walk never reloads the page under the recruiter", () => {
  const { planResumeRecovery: plan, RESUME_RECOVERY, RESUME_SCAN_STREAK_LIMIT } = Applicants;

  // THE INVARIANT THAT PROTECTS EVERY HEALTHY RUN, and it did not move when the
  // limit did. Delete this and a policy built to rescue a broken page is free to
  // reload a working one — mid-walk, throwing away the list position and the
  // pager page of a run that had nothing wrong with it, on a tab the recruiter
  // is reading.
  //
  // AMENDED FROM 3.10.0: the limit was 3, so the old sweep ran over streaks 0, 1
  // and 2. With a limit of 1 only streak 0 is below it, and a sweep over one
  // value is the kind of test that looks like coverage and is not. It is
  // rewritten to assert the thing it was always protecting — A WALK THAT HAS MET
  // NO NOTICE AT ALL MUST NEVER RELOAD — against every other input that could
  // possibly excuse one.
  assert.equal(RESUME_SCAN_STREAK_LIMIT, 1,
    "one: the notice is answered on the applicant that shows it (3.11.0)");

  for (const owed of [0, 1, 4, 25]) {
    for (const reloads of [0, 1, 11]) {
      for (const applicantReloads of [0, 1, 2]) {
        for (const fruitless of [0, 1, 3]) {
          const verdict = plan({ phase: "walking", streak: 0, owed, reloads, applicantReloads, fruitless });
          assert.equal(verdict.action, RESUME_RECOVERY.CONTINUE,
            `no notice means no reload, whatever else is true (${owed}/${reloads}/${applicantReloads}/${fruitless})`);
          assert.equal(verdict.reason, "",
            "and it is the ordinary no-op, not a refusal — the caller branches on that reason");
        }
      }
    }
  }
  ...
  // been opened — so only a notice may trigger a reload before the end.
  ...
  // AND THE SWEEP MUST NOT PASS VACUOUSLY. If someone raises the limit back to
  // 3 to "calm it down", the assertion above fails first and this one says what
  // the limit is for. Both halves are needed: the constant, and the behaviour.
  assert.equal(plan({ phase: "walking", streak: 1 }).action, RESUME_RECOVERY.RELOAD,
    "one notice reloads, or the recruiter's report is back");
  assert.equal(plan({ phase: "walking", streak: RESUME_SCAN_STREAK_LIMIT }).action, RESUME_RECOVERY.RELOAD);

  // A caller may still raise its own bar without touching the constant, and a
  // streak below THAT is just as quiet.
  assert.equal(plan({ phase: "walking", streak: 2, streakLimit: 3 }).action, RESUME_RECOVERY.CONTINUE,
    "the limit is the caller's to raise, and below it nothing happens");
});

test("3.13.0: PERMANENT — no number of reloads stops a job that is still owed resumes", () => {
  const { planResumeRecovery: plan, RESUME_RECOVERY } = Applicants;

  /**
   * THE ASSERTION THAT REPLACED THE CEILING, and it is the exact opposite of the
   * one that used to stand here.
   *
   * What stood here was `MAX_RESUME_RELOADS`, a job-wide lifetime budget of
   * twelve, asserted as an absolute ceiling "whatever the page still owes". It
   * was removed because of what it did on a real list: *"the pages were
   * reloading ... for at least first 100 in my case after it whenever the
   * warning/error came the pages were not reloading i waited for it to reload at
   * some point but even after 10 applicants it did not reload"*.
   *
   * Twelve reloads across a hundred applicants is one notice every eight, which
   * is an ordinary rate — so the ceiling was reached around applicant 100 and
   * never reset for the rest of the job. And it only ever bit runs where
   * reloading was WORKING: a hopeless page was stopped sooner by the breaker,
   * while a page whose reloads kept succeeding spent all twelve and went silent.
   *
   * The instruction that removed it: *"do not set any limiter"*. So this test
   * exists to make sure nobody quietly puts one back, and it sweeps absurd
   * numbers precisely because any reintroduced cap would be smaller than these.
   */
  for (const reloads of [0, 1, 12, 13, 100, 5000, 1e6]) {
    for (const phase of ["walking", "finished"]) {
      const triggering = phase === "finished" ? { owed: 5 } : { streak: 1, owed: 5 };
      const verdict = plan({ phase, reloads, ...triggering });
      assert.equal(verdict.action, RESUME_RECOVERY.RELOAD,
        `${reloads} reloads already spent must not stop a job that is still owed files`);
    }
  }

  // And no caller may reintroduce one through the options bag either: there is
  // no `maxReloads` parameter left to pass.
  assert.equal(plan({ phase: "finished", owed: 5, reloads: 999, maxReloads: 2 }).action,
    RESUME_RECOVERY.RELOAD,
    "a ceiling offered by a caller is not honoured, because there is no ceiling");
});
...
test("3.10.0: PERMANENT — canReload:false beats every trigger, because it carries three rules at once", () => {
  ...
  //
  // SWEPT OVER THE BOUNDS THAT EXIST NOW (3.11.0). This loop used to sweep
  // `recovered`, which the policy no longer reads at all, so a third of its
  // nesting was exercising one value dressed up as three. `applicantReloads` and
  // `fruitless` replace it, and the veto must beat both of them in either
  // direction — including the cases where a bound would have said GIVE_UP, which
  // is the interesting half: a refusal is not a settlement.
  for (const phase of ["walking", "finished"]) {
    for (const owed of [0, 1, 4, 25]) {
      for (const applicantReloads of [0, 2, 9]) {
        for (const fruitless of [0, 3]) {
          for (const reloads of [0, 1, 12, 20]) {
            for (const streak of [0, 1, 9]) {
              const verdict = plan({
                phase, owed, applicantReloads, fruitless, reloads, streak, canReload: false
              });
              assert.equal(verdict.action, RESUME_RECOVERY.CONTINUE,
...
test("3.10.0: PERMANENT — a refused reload is distinguishable from a healthy page", () => {
  ...
  for (const input of [
    ...
    { phase: "finished", owed: 25, reloads: 1 },
    // A single notice, which IS the trigger from 3.11.0 onwards.
    { phase: "walking", streak: 1 },
    { phase: "walking", streak: 9, reloads: 1 },
    // And the cases where a bound would otherwise have answered GIVE_UP: the
    // veto outranks them, and CONTINUE-with-a-reason is what keeps the job
    // re-armable when the tab comes back.
    { phase: "walking", streak: 1, applicantReloads: 2 },
    { phase: "finished", owed: 4, fruitless: 3 }
  ]) {
...
test("3.11.0: PERMANENT — give-up and reload are different answers, and only one is terminal", () => {
  ...
  // branching on them is branching on something real. Each of the three bounds
  // 3.11.0 added or moved produces the terminal one, and each names itself —
  // "it stopped reloading" is the report this release exists to answer, and it
  // is unanswerable if the three collapse into one string.
  assert.equal(plan({ phase: "finished", owed: 4, reloads: 0 }).action, RESUME_RECOVERY.RELOAD);
  const reasons = [
    plan({ phase: "walking", streak: 1, applicantReloads: 2 })
  ];
  for (const verdict of reasons) {
    assert.equal(verdict.action, RESUME_RECOVERY.GIVE_UP);
    assert.ok(verdict.reason, "a terminal answer always says which bound produced it");
  }

  // ONE PRODUCER OF `give-up`, WHERE THERE WERE THREE. The job-wide ceiling and
  // the fruitless breaker are gone, so the only terminal answer left is "this
  // applicant has had its two, walk on" — which is a scheduling decision about
  // one person, never a verdict on the job. That is the whole point of removing
  // the other two: nothing can now decide the JOB is finished with resumes it is
  // still owed.
  assert.equal(plan({ phase: "finished", owed: 4, reloads: 1e6 }).action, RESUME_RECOVERY.RELOAD,
    "no number of reloads produces a terminal answer any more");
});
...
test("3.10.0: PERMANENT — the lease carries the spent budget through a claim and a settle", () => {
  ...
  const spent = noteResumeReload(
    ...
    { now, applicantKey: "alice", recovered: 0 }
  ).entry;
  assert.equal(spent.resumeReloads, 1);
  // THE THREE 3.11.0 COUNTERS RIDE ON THE SAME LEASE FOR THE SAME REASON, and
  // are asserted through the same claim and settle. A spread that dropped these
  // would leave the per-applicant bound reading zero for ever — every reload
  // would look like this applicant's first, which is the loop the bound exists
  // to prevent — and it would fail silently, on a live page only.
  assert.equal(spent.resumeReloadKey, "alice");
  assert.equal(spent.resumeReloadKeyCount, 1);
  // `resumeReloadFruitless` used to ride here too and was removed with the
  // breaker it fed: it counted reloads that recovered nothing and disarmed the
  // whole job at three, which is a count, and counts are what kept stopping this
  // feature partway down a long list.
  assert.equal(spent.resumeReloadFruitless, undefined,
    "the fruitless counter is gone, not merely unused");
  ...
  assert.deepEqual(
    Applicants.readResumeReloadState(claimed.entry, { applicantKey: "alice" }),
    { reloads: 1, applicantReloads: 1 },
    "including which applicant it was spent on, which only matters across a reload"
  );
  ...
  assert.equal(interrupted.entry.resumeReloadKeyCount, 1, "nor a fresh turn for this applicant");
  ...
  assert.equal(rearmed.resumeReloads, 0);
  assert.deepEqual(Applicants.readResumeReloadState(rearmed, { applicantKey: "alice" }),
    { reloads: 0, applicantReloads: 0 },
    "every counter starts over, because the recruiter asking again is a new instruction");
});

test("3.11.0: junk input never invents a reload, and never disables a bound", () => {
  const { planResumeRecovery: plan, RESUME_RECOVERY } = Applicants;

  // This policy is fed by counters that come from a live walk and from a lease
  // that crossed a message boundary, and a walk that went wrong is exactly when
  // they arrive as undefined, as a string, or as something decremented past
  // zero. Erring towards CONTINUE is the safe direction in every one of those
  // cases: the cost of not reloading is resumes this run could have had, and the
  // cost of reloading on nonsense is the recruiter's page moving under them for
  // no reason at all.
  ...
  assert.deepEqual(plan({ phase: "finished", owed: -4, reloads: -2, fruitless: -1, applicantReloads: -9 }),
    { action: RESUME_RECOVERY.CONTINUE, reason: "", owed: 0, reloads: 0 },
    "owing minus four resumes is owing none");

  // Unparseable counters are none, not NaN — a comparison against NaN is false
  // in both directions and would make every ceiling unenforceable.
  assert.equal(
    plan({ phase: "nonsense", owed: "many", streak: "lots", reloads: "x", fruitless: "y", applicantReloads: "z" }).action,
  ...
  assert.deepEqual(plan({ phase: "finished", owed: "4", reloads: "1" }),
  ...
  assert.equal(plan({ phase: "walking", streak: "1", applicantReloads: "2" }).action, RESUME_RECOVERY.GIVE_UP,
    "and the one bound left is enforced against a string too");
  // A string that would once have hit the ceiling now reloads, because there is
  // no ceiling to hit. Asserted rather than deleted: if one is ever put back,
  // this is where it shows up.
  assert.equal(plan({ phase: "finished", owed: "4", reloads: "12" }).action, RESUME_RECOVERY.RELOAD,
    "twelve is an ordinary number now, not a wall");

  // A JUNK *LIMIT* MUST FAIL CLOSED, WHICH IS THE OPPOSITE DIRECTION FROM A JUNK
  // COUNTER AND IS WORTH ITS OWN ASSERTION. A counter that cannot be read is
  // "nothing has happened yet"; a LIMIT that cannot be read is a bound that has
  // gone missing, and reading it as infinity would quietly delete the only thing
  // standing between this feature and a page that reloads for ever. Every one of
  // them clamps to zero, which spends nothing.
  //
  // ONE BOUND LEFT TO TEST, where there were three: `maxReloads` and
  // `maxFruitless` were deleted with the ceilings they capped, and a caller
  // passing either now gets an ordinary reload rather than a refusal — asserted
  // in the ceiling test, so that a reintroduced cap fails loudly there.
  for (const limits of [
    { maxApplicantReloads: "x" },
    { maxApplicantReloads: null },
    { maxApplicantReloads: NaN },
    { maxApplicantReloads: -3 }
  ]) {
    const verdict = plan({ phase: "walking", streak: 1, owed: 1, ...limits });
    assert.equal(verdict.action, RESUME_RECOVERY.GIVE_UP,
      `an unreadable bound spends nothing (${JSON.stringify(limits)})`);
    assert.notEqual(verdict.action, RESUME_RECOVERY.RELOAD,
      "a missing bound must never read as an unlimited one");
  }
});

test("3.11.0: PERMANENT — one applicant may not reload the page for ever", () => {
  const { planResumeRecovery: plan, RESUME_RECOVERY, MAX_APPLICANT_RESUME_RELOADS } = Applicants;

  // THE BOUND THAT MAKES RELOAD-ON-SIGHT SAFE, and the one that took over the
  // job the streak of three used to do badly.
  //
  // The failure it prevents is a loop, and the loop is real rather than
  // theoretical: LinkedIn is genuinely still scanning one applicant's file, the
  // notice appears, the page reloads, the resumed run goes straight back to the
  // applicants it still owes — which is that applicant — the file is STILL being
  // scanned, and the trigger is satisfied again. Nothing in the job ceiling or
  // the fruitless breaker stops that quickly; both are job-wide and would let
  // one stubborn file spend everyone else's budget first.
  assert.equal(MAX_APPLICANT_RESUME_RELOADS, 2,
    "two: the first covers a stale attachment session, the second a scan that had not finished");

  for (let spent = 0; spent < MAX_APPLICANT_RESUME_RELOADS; spent += 1) {
    assert.equal(plan({ phase: "walking", streak: 1, owed: 3, applicantReloads: spent }).action,
      RESUME_RECOVERY.RELOAD,
      `${spent} spent on this applicant still leaves them a turn`);
  }
  for (const spent of [MAX_APPLICANT_RESUME_RELOADS, MAX_APPLICANT_RESUME_RELOADS + 5]) {
    const verdict = plan({ phase: "walking", streak: 1, owed: 3, applicantReloads: spent });
    assert.equal(verdict.action, RESUME_RECOVERY.GIVE_UP, "and then the walk moves past them");
    assert.equal(verdict.reason, "this applicant's reloads are spent",
      "named specifically, because 'the job is out of budget' would be the wrong thing to tell the user");
  }

  // GIVING UP ON THE RELOAD IS NOT GIVING UP ON THE APPLICANT. The verdict still
  // reports what is owed, the caller leaves them in `resumesOwed`,
  // `hasPendingResume` keeps them out of the collected index, and the next run
  // goes back for them. This assertion is the one that stops a future reading of
  // "give up" as "mark them done".
  assert.equal(plan({ phase: "walking", streak: 1, owed: 7, applicantReloads: 2 }).owed, 7,
    "the debt survives the refusal to reload again for this person");

  // It is asked BEFORE the job ceiling, so the run reports the specific cause
  // rather than the general one when both are true.
  assert.equal(plan({ phase: "walking", streak: 1, applicantReloads: 2, reloads: 12 }).reason,
    "this applicant's reloads are spent",
    "the most specific bound that fired is the one reported");
});

test("3.11.0: PERMANENT — the end of the walk is exempt from the per-applicant bound", () => {
  const { planResumeRecovery: plan, RESUME_RECOVERY } = Applicants;

  // WHY THE EXEMPTION EXISTS. An end-of-walk reload is not FOR anybody in
  // particular — it is for everyone still owed a CV after the whole list has
  // been walked. There is no applicant to charge it to, and charging it to
  // whoever happened to be last would let ONE stubborn file veto the reload the
  // other twenty applicants needed. The caller sends no key for this reason; the
  // policy ignores the bound for the same one.
  for (const applicantReloads of [0, 2, 99]) {
    assert.equal(plan({ phase: "finished", owed: 4, applicantReloads }).action, RESUME_RECOVERY.RELOAD,
      "a finished walk still goes back for everyone it owes");
  }

  // AND NOW NOTHING ELSE IS IN ITS WAY EITHER. The job ceiling and the fruitless
  // breaker used to apply in both phases and are gone, so the end of the walk
  // genuinely does answer to nothing except Stop — which is `canReload`, tested
  // separately, and is a person rather than a counter.
  assert.equal(plan({ phase: "finished", owed: 4, applicantReloads: 99, reloads: 1e6 }).reason,
    "resumes-still-owed",
    "a million reloads in, a job still owed files still goes back for them");

  // And the same inputs mid-walk DO hit the bound, so the exemption is really
  // about the phase rather than about the numbers.
  assert.equal(plan({ phase: "walking", streak: 1, owed: 4, applicantReloads: 2 }).action,
    RESUME_RECOVERY.GIVE_UP, "mid-walk the same applicant is out of turns");
});

test("3.11.0: the lease counts reloads per applicant, and forgets nothing across the reload", () => {
  const { noteResumeReload, readResumeReloadState, createAutoRunEntry } = Applicants;

  // THE BOOKKEEPING THE POLICY ABOVE IS ONLY AS GOOD AS. Every counter it reads
  // lives on the auto-run lease, because the thing they bound — a page reload —
  // destroys the document and every counter inside it.
  const fresh = createAutoRunEntry({ options: {}, now: "2026-08-25T09:00:00.000Z", runId: "r1", tabId: 7 });
  assert.deepEqual(readResumeReloadState(fresh, { applicantKey: "alice" }),
    { reloads: 0, applicantReloads: 0 },
    "a new instruction starts with its whole budget");

  // The same applicant's count climbs.
  let entry = noteResumeReload(fresh, { now: "n1", applicantKey: "alice", recovered: 0 }).entry;
  assert.deepEqual(readResumeReloadState(entry, { applicantKey: "alice" }),
    { reloads: 1, applicantReloads: 1 });
  entry = noteResumeReload(entry, { now: "n2", applicantKey: "alice", recovered: 0 }).entry;
  assert.deepEqual(readResumeReloadState(entry, { applicantKey: "alice" }),
    { reloads: 2, applicantReloads: 2 },
    "two reloads charged to the same person, which is exactly their limit");

  // ...AND ONLY THEIRS. Read as somebody else, the per-applicant count is zero:
  // the next applicant starts with their own full turn, or one stale file would
  // silently disarm the remedy for the whole rest of the list.
  assert.equal(readResumeReloadState(entry, { applicantKey: "bob" }).applicantReloads, 0,
    "a different applicant is not charged for this one's reloads");
  assert.equal(readResumeReloadState(entry, { applicantKey: "bob" }).reloads, 2,
    "while the JOB total is shared by everybody, because it is a count of the job");

  // A different key restarts the count at one rather than continuing it.
  entry = noteResumeReload(entry, { now: "n3", applicantKey: "bob", recovered: 0 }).entry;
  assert.equal(readResumeReloadState(entry, { applicantKey: "bob" }).applicantReloads, 1);
  assert.equal(readResumeReloadState(entry, { applicantKey: "alice" }).applicantReloads, 0,
    "one key and one count — alice is forgotten once bob is the one being reloaded for");

  // THE FRUITLESS STREAK IS NOT COUNTED ANY MORE, and the lease no longer carries
  // it. It measured reloads that recovered nothing and disarmed the whole job at
  // three - which is a count, and a count on a job cannot know how many
  // applicants are left. Asserted as absent rather than simply not mentioned, so
  // that reintroducing it has to pass through this test.
  assert.equal(readResumeReloadState(entry, { applicantKey: "bob" }).fruitless, undefined,
    "nothing reports a fruitless streak, because nothing counts one");
  assert.equal(entry.resumeReloadFruitless, undefined, "and the lease does not carry one");

  // An END-OF-WALK reload passes no key. It must CLEAR the key rather than
  // charge itself to whoever was last, or a sweep for everybody would count
  // against one person's limit.
  const swept = noteResumeReload(entry, { now: "n5", applicantKey: "", recovered: 0 }).entry;
  assert.equal(readResumeReloadState(swept, { applicantKey: "bob" }).applicantReloads, 0,
    "the sweep is nobody's fault in particular");
  assert.equal(readResumeReloadState(swept, { applicantKey: "" }).applicantReloads, 0,
    "and an empty key never matches an empty key either, or every applicant would share one count");
  assert.equal(swept.resumeReloads, 4, "while the job total still counts it, because it is a reload");

  // Monotonic in the job total. Nothing compares it to a ceiling any more - it
  // is kept because the reload is only taken once the count is PROVEN to have
  // risen by exactly one, which is what stops a reload nothing recorded from
  // repeating for ever now that no ceiling would catch it.
  assert.equal(noteResumeReload(swept, { now: "n6" }).entry.resumeReloads, 5,
    "it keeps counting, and never saturates");

  // A lease that is not a lease is refused rather than invented.
  for (const junk of [null, undefined, 0, "", "entry", 5, true]) {
    const missed = noteResumeReload(junk);
    assert.equal(missed.changed, false, `${JSON.stringify(junk)} is not a lease`);
    assert.equal(missed.reason, "missing", "and it says so rather than inventing one");
  }
  // Reading one, on the other hand, must answer zeroes rather than throw: the
  // worker asks this of whatever storage gave back.
  assert.deepEqual(readResumeReloadState(null, { applicantKey: "alice" }),
    { reloads: 0, applicantReloads: 0 });
});

test("3.13.0: PERMANENT — a long list is walked to the end, however many notices it shows", () => {
  const {
  ...
  /**
   * DRIVEN THROUGH THE REAL PERSISTED CYCLE — read the lease, plan, spend, come
   * back as a new document — rather than asserted against a hand-computed
   * number, because a hand-computed expectation is exactly how a ceiling gets
   * reintroduced without anyone noticing.
   *
   * The shape being proved is the recruiter's list: hundreds of applicants, a
   * notice on many of them, and NO point at which the remedy switches itself
   * off. The old ceiling failed this at applicant 100.
   */
  const walk = (applicants) => {
    let entry = createAutoRunEntry({ options: {}, now: "t", runId: "r", tabId: 1 });
    let reloads = 0;
    let helped = 0;
    for (let i = 0; i < applicants; i += 1) {
      const key = `applicant-${i}`;
      // Every applicant shows the notice once, which is the worst realistic case.
      const state = readResumeReloadState(entry, { applicantKey: key });
      const verdict = plan({
        phase: "walking", streak: 1, owed: applicants - i,
        reloads: state.reloads, applicantReloads: state.applicantReloads
      });
      if (verdict.action === RESUME_RECOVERY.RELOAD) {
        entry = noteResumeReload(entry, { now: "t", applicantKey: key }).entry;
        reloads += 1;
        helped += 1;
      }
    }
    return { reloads, helped, spent: entry.resumeReloads };
  };

  for (const size of [10, 100, 500, 2000]) {
    const run = walk(size);
    assert.equal(run.helped, size,
      `every one of ${size} applicants must get a reload — no count may stop it partway`);
    assert.equal(run.spent, size, "and the lease counts them all rather than saturating");
  }

  // The per-applicant rule still bites, and it is the ONE bound left: a single
  // stuck file gets two reloads and then the walk moves past it. That is not a
  // cap on the feature — without it the run reloads forever on applicant N and
  // never reaches N+1, which is the opposite of collecting every profile.
  assert.equal(plan({ phase: "walking", streak: 1, owed: 9, applicantReloads: 2 }).action,
...
// ---------------------------------------------------------- 3.14.0 messaging
// The user decided, having been shown what it costs, that this extension may
// press two controls it has always refused: the applicant panel's `Message` and
// the composer's `Send`. Rule 5 was amended for them. Everything below is the
// proof that the amendment is exactly two label/purpose pairs wide and that
// nothing else moved.

test("3.14.0: the denylist says WHICH word fired, so one word can be forgiven at one purpose", () => {
  const { CONTROL_PURPOSE, firedForbiddenWords, classifyApplicantControl } = Applicants;

  // The reader first, because the whole carve-out is built on it. A boolean
  // cannot tell `Send`, which is the composer's own button, from `Send InMail`,
  // which is an InMail control that begins with the word Send.
  assert.deepEqual(firedForbiddenWords("Message"), ["message"]);
  assert.deepEqual(firedForbiddenWords("Send"), ["send"]);
  assert.deepEqual(firedForbiddenWords("Send InMail"), ["send", "inmail"]);
  assert.deepEqual(firedForbiddenWords("Message and reject"), ["message", "reject"]);
  assert.deepEqual(firedForbiddenWords("Download resume"), [], "a control that fires nothing reports nothing");

  // It is compiled from the SAME string as the boolean, so a word added to the
  // denylist tomorrow is refused inside the composer without anyone having to
  // remember that a second copy of the list exists. Asserted by walking the
  // words the boolean knows rather than by re-listing them here.
  for (const label of ["Reject", "Shortlist", "Move to", "Good fit", "Add a note", "Try Premium", "Email applicant"]) {
    assert.equal(Applicants.FORBIDDEN_APPLICANT_CONTROL_PATTERN.test(label.toLowerCase()), true);
    assert.ok(firedForbiddenWords(label).length > 0, `"${label}" fires the boolean but names no word`);
  }

  // A `g` regex carries `lastIndex`. If the reader ever shared one object with
  // the boolean test, a control's verdict would depend on the label judged
  // before it — a button refused once and allowed the next time round the list.
  assert.notEqual(Applicants.FIRED_FORBIDDEN_ACTION_PATTERN, Applicants.FORBIDDEN_APPLICANT_CONTROL_PATTERN);
  assert.equal(Applicants.FORBIDDEN_APPLICANT_CONTROL_PATTERN.global, false);
  assert.equal(Applicants.FIRED_FORBIDDEN_ACTION_PATTERN.global, true);
  for (let round = 0; round < 3; round += 1) {
    assert.deepEqual(firedForbiddenWords("Send InMail"), ["send", "inmail"],
      "the same label must give the same answer however many labels came before it");
  }

  // THE TRUTH TABLE. Each row is a label, the purpose it is asked for, and
  // whether it may be pressed — and the rows that refuse are the argument.
  const verdict = (text, purpose) => classifyApplicantControl({ text, purpose, inContainer: true });

  // Forgiven: the word fired is the one word this purpose may forgive.
  assert.equal(verdict("Message", CONTROL_PURPOSE.MESSAGE_OPEN).allowed, true);
  assert.equal(verdict("Message", CONTROL_PURPOSE.MESSAGE_OPEN).reason, "message-open");
  assert.equal(verdict("Send", CONTROL_PURPOSE.MESSAGE_SEND).allowed, true);
  assert.equal(verdict("Send", CONTROL_PURPOSE.MESSAGE_SEND).reason, "message-send");

  // Refused, because a second word fired and the exemption is TOTAL: one word
  // the purpose may not forgive is enough, wherever in the label it sits.
  const inmail = verdict("Send InMail", CONTROL_PURPOSE.MESSAGE_SEND);
  assert.equal(inmail.allowed, false, "Send InMail is an InMail control that starts with Send");
  assert.equal(inmail.forbidden, true, "and it is refused BY THE DENYLIST, not merely unmatched");
  const compound = verdict("Message and reject", CONTROL_PURPOSE.MESSAGE_OPEN);
  assert.equal(compound.allowed, false);
  assert.equal(compound.forbidden, true, "`reject` is on nobody's exemption");

  // Refused, because the exemption belongs to the PURPOSE and not to the word.
  const wrongWay = verdict("Message", CONTROL_PURPOSE.MESSAGE_SEND);
  assert.equal(wrongWay.allowed, false, "a Message control found inside a composer is not a Send control");
  assert.equal(wrongWay.forbidden, true);
  const otherWay = verdict("Send", CONTROL_PURPOSE.MESSAGE_OPEN);
  assert.equal(otherWay.allowed, false, "a Send sitting in the applicant panel is still the forbidden action it was");
  assert.equal(otherWay.forbidden, true);

  // Refused everywhere else, exactly as before this release.
  for (const purpose of [
    CONTROL_PURPOSE.CONTACT, CONTROL_PURPOSE.CONTACT_MENU, CONTROL_PURPOSE.RESUME,
    CONTROL_PURPOSE.RESUME_DOWNLOAD, CONTROL_PURPOSE.DISCLOSURE, CONTROL_PURPOSE.PAGINATION
  ]) {
    for (const label of ["Message", "Send"]) {
      const refused = verdict(label, purpose);
      assert.equal(refused.allowed, false, `"${label}" leaked at purpose ${purpose}`);
      assert.equal(refused.forbidden, true, `"${label}" must stay denylisted at purpose ${purpose}`);
    }
  }

  // **AND THE ROW IS BYTE-FOR-BYTE WHAT IT WAS.** A row forgives nothing, so
  // the anchored rule still decides it and a person's card still opens.
  const rowHref = "https://www.linkedin.com/hiring/applicants/?jobId=4277798308&applicationId=25550787924";
  const asRow = (text) => classifyApplicantControl({
    text, purpose: CONTROL_PURPOSE.APPLICANT_ROW, inContainer: true, href: rowHref
  });
  assert.equal(asRow("Message").allowed, false, "a whole label of `Message` is a control, not a person");
  assert.equal(asRow("Message").forbidden, true);
  assert.equal(asRow("Komal Sharma Senior Engineer Good fit").allowed, true,
    "and a rated applicant's card still opens — 3.9.4 is untouched");
});

/**
 * PERMANENT. This is the boundary of the 3.14.0 carve-out, and the carve-out is
 * the only time this project has ever allowed a control that CONTACTS somebody.
 *
 * The exemption is stated as one map: which purpose may forgive which word. A
 * third entry, a second word on either list, or a widening of either whole-label
 * rule would let this extension press something the user has not agreed to —
 * and none of those would fail any other test in this file, because every other
 * test asks whether a specific label is refused rather than whether the set of
 * allowed ones has grown. This test asks the second question.
 *
 * Do not delete or weaken it. If the exemption must genuinely grow, rule 5 in
 * CLAUDE.md changes first, in its own task, and this test changes with it.
 */
test("3.14.0: PERMANENT — the message exemption is two label/purpose pairs and reaches nothing else", () => {
  const { CONTROL_PURPOSE, PURPOSE_EXEMPT_WORDS, classifyApplicantControl } = Applicants;

  // 1. THE MAP ITSELF. Two purposes, one word each, and both frozen — so a
  //    third exemption is a source change somebody has to justify, never a
  //    line quietly pushed onto an array at runtime.
  assert.deepEqual(
    Object.keys(PURPOSE_EXEMPT_WORDS).sort(),
    [CONTROL_PURPOSE.MESSAGE_OPEN, CONTROL_PURPOSE.MESSAGE_SEND].sort(),
    "no purpose but the two message purposes may forgive anything"
  );
  assert.deepEqual(PURPOSE_EXEMPT_WORDS[CONTROL_PURPOSE.MESSAGE_OPEN], ["message"]);
  assert.deepEqual(PURPOSE_EXEMPT_WORDS[CONTROL_PURPOSE.MESSAGE_SEND], ["send"]);
  assert.equal(Object.isFrozen(PURPOSE_EXEMPT_WORDS), true);
  for (const words of Object.values(PURPOSE_EXEMPT_WORDS)) assert.equal(Object.isFrozen(words), true);

  // 2. NEITHER WORD LEFT THE DENYLIST. This is what makes it an exemption
  //    rather than a deletion: `message` and `send` are still forbidden actions,
  //    and every purpose added after this one inherits that refusal for free.
  for (const words of Object.values(PURPOSE_EXEMPT_WORDS)) {
    for (const word of words) {
      assert.equal(Applicants.FORBIDDEN_APPLICANT_CONTROL_PATTERN.test(word), true,
        `"${word}" must stay on FORBIDDEN_APPLICANT_ACTIONS — the carve-out forgives, it does not delete`);
    }
  }

  // 3. THE SWEEP. Every purpose the enum knows, against the four labels this
  //    release is about. Exactly two cells may be allowed; every other cell in
  //    the grid must refuse, and a purpose invented tomorrow joins the grid
  //    automatically because the loop reads the enum rather than a list.
  const EXEMPT_PAIRS = new Map([
    [CONTROL_PURPOSE.MESSAGE_OPEN, "Message"],
    [CONTROL_PURPOSE.MESSAGE_SEND, "Send"]
  ]);
  const LABELS = ["Message", "Send", "Send InMail", "Message and reject"];
  let allowedCells = 0;
  for (const purpose of Object.values(CONTROL_PURPOSE)) {
    for (const label of LABELS) {
      const verdict = classifyApplicantControl({ text: label, purpose, inContainer: true });
      if (EXEMPT_PAIRS.get(purpose) === label) {
        // Asserted rather than skipped, so the test cannot pass by the
        // exemption having quietly stopped working.
        assert.equal(verdict.allowed, true, `the one allowed pairing "${label}" @ ${purpose} must still work`);
        allowedCells += 1;
        continue;
      }
      assert.equal(verdict.allowed, false, `"${label}" leaked at purpose ${purpose}`);
  ...
  assert.equal(allowedCells, 2, "exactly two cells in the grid, and they are the two rule 5 names");

  // 4. AND THE CONTAINER PROOF IS NOT OPTIONAL. A Message loose on a hiring
  //    page belongs to LinkedIn's own messaging overlay, and a Send loose on
  //    one belongs to something that is not this composer.
  const loose = (text, purpose) => classifyApplicantControl({ text, purpose, inContainer: false });
  assert.equal(loose("Message", CONTROL_PURPOSE.MESSAGE_OPEN).allowed, false);
  assert.equal(loose("Message", CONTROL_PURPOSE.MESSAGE_OPEN).reason, "outside-applicant-panel");
  assert.equal(loose("Send", CONTROL_PURPOSE.MESSAGE_SEND).allowed, false);
  assert.equal(loose("Send", CONTROL_PURPOSE.MESSAGE_SEND).reason, "outside-message-composer");

  // 5. A caller-supplied purpose reaches the map as a property NAME, so an
  //    inherited key must not read as an exemption. `constructor` returns a
  //    function; `Array.isArray` is what stops the gate throwing — or worse,
  //    forgiving — inside the one branch that must never fail open.
  for (const purpose of ["constructor", "toString", "__proto__", "hasOwnProperty"]) {
    const verdict = classifyApplicantControl({ text: "Message", purpose, inContainer: true });
    assert.equal(verdict.allowed, false, `purpose "${purpose}" must not inherit an exemption`);
    assert.equal(verdict.forbidden, true);
  }
});

test("3.14.0: both message allowlists are whole-label rules, and Send is a single literal", async () => {
  const { CONTROL_PURPOSE, classifyApplicantControl } = Applicants;
  const opens = (text) =>
    classifyApplicantControl({ text, purpose: CONTROL_PURPOSE.MESSAGE_OPEN, inContainer: true }).allowed;
  const sends = (text) =>
    classifyApplicantControl({ text, purpose: CONTROL_PURPOSE.MESSAGE_SEND, inContainer: true }).allowed;

  // What LinkedIn actually paints: the bare verb, and the verb with the
  // applicant's name once the screen-reader half of the label is folded in.
  assert.equal(opens("Message"), true);
  assert.equal(opens("Message Gaurang"), true);
  assert.equal(opens("Message Gaurang Kumar"), true);
  assert.equal(opens("Message Sushmitha.L"), true);
  assert.equal(opens("Message O'Brien"), true);

  // A name is letters and digits, and so is `and`. Without the lookahead this
  // rule reads "Message and reject" as a Message control addressed to somebody
  // called "and reject". The denylist refuses that label too — this is the
  // second lock, and it catches the clause-joining words the denylist has no
  // opinion about.
  for (const label of [
    "Message and reject", "Message or skip", "Message then archive",
    "Message and forward to the team", "Message Gaurang and reject",
    "Message the applicant about the role today"
  ]) {
    assert.equal(opens(label), false, `"${label}" must not read as a Message control`);
  }

  // `Send` is the strictest allowlist in the file: one literal, no tail. Every
  // other control this extension presses can be pressed again if the first
  // press achieved nothing. A sent message has no undo.
  assert.equal(sends("Send"), true);
  for (const label of ["Send InMail", "Send message", "Send message to Gaurang", "Send now", "Resend"]) {
    assert.equal(sends(label), false, `"${label}" must not read as the composer's Send`);
  }

  // **THE ONE THAT THE DENYLIST DOES NOT CATCH, and the reason this pattern is
  // a literal rather than a `^send` prefix.** `\bconnect\b` does not match
  // inside "connection", so the only forbidden word "Send connection request"
  // fires is `send` — which is precisely the word MESSAGE_SEND forgives. The
  // whole-label rule is the only thing standing between this label and a click.
  assert.deepEqual(Applicants.firedForbiddenWords("Send connection request"), ["send"],
    "the denylist really does fire only the exempt word here — this is not a hypothetical");
  const invite = classifyApplicantControl({
    text: "Send connection request", purpose: CONTROL_PURPOSE.MESSAGE_SEND, inContainer: true
  });
  assert.equal(invite.allowed, false, "and it is still refused");
  assert.equal(invite.reason, "not-a-send-control", "by the allowlist, because the denylist could not");

  // The branches are textually BELOW the denylist gate, like every other
  // purpose. Order here is not style: the gate is what the exemption runs
  // inside, so a branch above it would answer before the denylist ever spoke.
  const core = await readFile(resolve(root, "src/applicants-core.js"), "utf8");
  const policy = core.slice(
    core.indexOf("function classifyApplicantControl"),
    core.indexOf('return refuse("unknown-purpose")')
  );
  assert.ok(
    policy.indexOf("firesOutsideExemption") < policy.indexOf("CONTROL_PURPOSE.MESSAGE_OPEN"),
    "the denylist gate is consulted before the Message branch"
  );
  assert.ok(
    policy.indexOf("firesOutsideExemption") < policy.indexOf("CONTROL_PURPOSE.MESSAGE_SEND"),
    "and before the Send branch"
  );
});

test("3.14.0: a message is opened, inserted, verified and only then sent — never for the wrong person", () => {
  const { MESSAGE_STEP, planMessageStep } = Applicants;
  const step = (options) => planMessageStep(options).action;

  // THE HAPPY PATH, driven rather than asserted step by step, because the
  // question worth answering is whether a real walk terminates — and in exactly
  // one send.
  const drive = (seed) => {
    const state = { ...seed };
    const walked = [];
    let sends = 0;
    for (let guard = 0; guard < 20; guard += 1) {
      const next = planMessageStep(state);
      walked.push(next.action);
      if (next.action === MESSAGE_STEP.DONE || next.action === MESSAGE_STEP.GIVE_UP) break;
      if (next.action === MESSAGE_STEP.OPEN) state.composerOpen = true;
      else if (next.action === MESSAGE_STEP.INSERT) state.textInserted = true;
      else if (next.action === MESSAGE_STEP.VERIFY) state.readBackMatches = true;
      else if (next.action === MESSAGE_STEP.SEND) { sends += 1; state.sent = true; }
      else if (next.action === MESSAGE_STEP.CONFIRM) state.confirmed = true;
    }
    return { walked, sends, state };
  };

  const happy = drive({});
  assert.deepEqual(happy.walked, [
    MESSAGE_STEP.OPEN, MESSAGE_STEP.INSERT, MESSAGE_STEP.VERIFY,
    MESSAGE_STEP.SEND, MESSAGE_STEP.CONFIRM, MESSAGE_STEP.DONE
  ], "open, insert, verify, send, confirm, done — and the verify is not optional");
  assert.equal(happy.sends, 1, "one send, and the ladder never offers a second");

  // WRONG PERSON, AT EVERY STAGE. LinkedIn routes ahead of its own render, so
  // the address bar names the next applicant while the panel still shows the
  // previous one — the same race `describePanelArrival` exists for. A composer
  // caught on it belongs to somebody else and must never be typed into,
  // whatever else is true, INCLUDING a message this run has already sent.
  const stages = [
    {},
    { composerOpen: true },
    { composerOpen: true, textInserted: true },
    { composerOpen: true, textInserted: true, readBackMatches: true },
    { composerOpen: true, textInserted: true, readBackMatches: true, sent: true },
    { composerOpen: true, textInserted: true, readBackMatches: true, sent: true, confirmed: true }
  ];
  for (const stage of stages) {
    assert.equal(step({ ...stage, wrongPerson: true }), MESSAGE_STEP.GIVE_UP,
      `a composer belonging to somebody else must stop the ladder at ${JSON.stringify(stage)}`);
  }

  // A FAILED READ-BACK NEVER REACHES SEND. A composer holding a truncated
  // message, or a template whose variables did not resolve, is not a message to
  // send at a lower confidence — and there is no undo to fall back on. `null`
  // (not asked yet) verifies; exactly `false` (asked, and the answer was no)
  // gives up. Nothing but exactly `true` sends.
  assert.equal(step({ composerOpen: true, textInserted: true }), MESSAGE_STEP.VERIFY,
    "not read back yet is a step, not a verdict");
  assert.equal(step({ composerOpen: true, textInserted: true, readBackMatches: null }), MESSAGE_STEP.VERIFY);
  assert.equal(step({ composerOpen: true, textInserted: true, readBackMatches: false }), MESSAGE_STEP.GIVE_UP,
    "a half-inserted message gives up; it is never sent");
  const failed = drive({ composerOpen: true, textInserted: true, readBackMatches: false });
  assert.equal(failed.sends, 0, "and a driven walk over it presses Send exactly never");
  assert.deepEqual(failed.walked, [MESSAGE_STEP.GIVE_UP]);

  // THE CALLER'S VETO — Stop, a rate limit, a cap, a person who must not be
  // messaged. Checked before the composer is opened, because there is no reason
  // to open one this run may not send from.
  assert.equal(step({ canSend: false }), MESSAGE_STEP.GIVE_UP);
  assert.equal(step({ composerOpen: true, canSend: false }), MESSAGE_STEP.GIVE_UP);
  assert.equal(step({ composerOpen: true, textInserted: true, readBackMatches: true, canSend: false }),
    MESSAGE_STEP.GIVE_UP, "a verified message is still not sent once the veto is in");
  assert.equal(drive({ canSend: false }).sends, 0);

  // ...but it does NOT discard a send that already happened. Rule 12: Stop ends
  // work, it never discards what that work produced. A message already gone is
  // a fact, and the only thing left to do with it is observe it and record it.
  assert.equal(step({ composerOpen: true, textInserted: true, readBackMatches: true, sent: true, canSend: false }),
    MESSAGE_STEP.CONFIRM, "a message already sent is confirmed, not abandoned unrecorded");

  // DONE AND GIVE_UP ARE TERMINAL. Their causes are facts the caller has
  // recorded, so feeding the same state back gives the same answer and a driven
  // walk always ends.
  const done = { composerOpen: true, textInserted: true, readBackMatches: true, sent: true, confirmed: true };
  assert.equal(step(done), MESSAGE_STEP.DONE);
  assert.equal(step(done), MESSAGE_STEP.DONE, "DONE is not left");
  assert.equal(step({ ...done, canSend: false }), MESSAGE_STEP.DONE, "and a late veto does not unfinish it");
  for (const gaveUp of [
    { wrongPerson: true },
    { canSend: false },
    { composerOpen: true, textInserted: true, readBackMatches: false }
  ]) {
    assert.equal(step(gaveUp), MESSAGE_STEP.GIVE_UP);
    assert.equal(step(gaveUp), MESSAGE_STEP.GIVE_UP, "GIVE_UP is not left");
  }

  // Every verdict says why, in its own words. The last two releases each spent
  // a round guessing at a defect one line of diagnostics would have named.
  const reasons = new Set([
    planMessageStep({}).reason,
    planMessageStep({ composerOpen: true }).reason,
    planMessageStep({ composerOpen: true, textInserted: true }).reason,
    planMessageStep({ composerOpen: true, textInserted: true, readBackMatches: true }).reason,
    planMessageStep({ composerOpen: true, textInserted: true, readBackMatches: false }).reason,
    planMessageStep({ canSend: false }).reason,
    planMessageStep({ wrongPerson: true }).reason,
    planMessageStep(done).reason
  ]);
  assert.equal(reasons.size, 8, "eight distinct states, eight distinct reasons");

  // And the ladder is pure: no DOM, no clock, no chrome. It is the only place
  // the send order can be ANSWERED rather than argued, because there is no
  // jsdom here and none of these questions is answerable on a live account.
  assert.equal(typeof planMessageStep, "function");
  assert.equal(Object.isFrozen(MESSAGE_STEP), true);
});
